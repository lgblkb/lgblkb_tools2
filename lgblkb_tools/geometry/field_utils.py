...
from lgblkb_tools.geometry.utils.geom_utils import min_dist
from .line_utils import TheLine
from .point_utils import ThePoint
from .poly_utils import ThePoly, TheChunk
...
class FieldPoly(ThePoly):
    ...
    def get_min_altitude(self, use_mp=False):
        if use_mp:
            alt_getter = lambda _exterior_line: (
                ...
                _exterior_line)
            altitude_data = ParallelTasker(alt_getter).set_run_params(_exterior_line=self.all_lines).run(
                sleep_time=1e-4)
    ...
    def calculate_area(self, crs):
        return SpatialMan.from_geoms([self.geometry], crs).area.sum()
...
def main():
    ...
    for poly_extent in [8e3, 15000]:
        ...
        for parcel_area in [50, 25, 5]:
            ...
            for i in range(5):
                field_poly = \
                    ParallelTasker(FieldPoly.synthesize, cities_count=50, poly_extent=poly_extent, hole_count=0) \
